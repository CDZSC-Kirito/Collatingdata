...
![3b7f14ac-4282-34ed-821e-e68d2228e18f](C:\Users\Administrator\Desktop\3b7f14ac-4282-34ed-821e-e68d2228e18f.png)
...
![BM-2](C:\Users\Administrator\Desktop\BM-2.png)

 （坏字符c，没有出现模式串P中，直接将P移动c的下一个位置）

2.如果坏字符出现在模式串中，则将模式串最靠近好后缀的坏字符（当然这个实现就有点繁琐）与母串的坏字符对齐：

![BM-3](C:\Users\Administrator\Desktop\BM-3.png)
...
![BM-4](C:\Users\Administrator\Desktop\BM-4.png)

 2.模式串中没有子串匹配上后后缀，此时需要寻找模式串的一个最长前缀，并让该前缀等于好后缀的后缀，寻找到该前缀后，让该前缀和好后缀对齐即可。

![BM-5](C:\Users\Administrator\Desktop\BM-5.png)
...
![最小表示法-1](C:\Users\Administrator\Desktop\最小表示法-1.PNG)
...
![最小表示法-2](F:\整理素材\最小表示法-2.PNG)

1. 设$u=s1+s1$，$w=s2+s2$并设指针$i$,$j$指向$u$,$w$第一个字符 ![最小表示法-3](F:\整理素材\最小表示法-3.PNG)

   ​

2. 如果$s1$和$s2$是循环同构的，那么当$i$,$j$分别指向$M(s1)$,$M(s2)$时，一定可以得到$u[i→i+|s1|-1]=w[j→j+|s2|-1]$，迅速输出正确解。

   ![最小表示法-4](F:\整理素材\最小表示法-4.PNG)
